...
import { getEssay, getVisibleEssays } from '@/lib/writing'
...
export async function generateStaticParams() {
  return getVisibleEssays().map((e) => ({ slug: e.slug }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!essay) return {}
  return {
    title: `${essay.title} — kyleisun.cool`,
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      ...
      <header className="max-w-2xl mb-14">
        ...
        <p className="font-mono text-xs text-muted-soft mt-6">{essay.date}</p>
      </header>
